...
class CANopenJointStatePublisher : public rclcpp::Node {
 public:
  CANopenJointStatePublisher() : Node("canopen_joint_state_publisher") {
    ...
    if (!kdl_parser::treeFromUrdfModel(model, tree)) {
      throw std::runtime_error(
          "Failed to extract kdl tree from robot description");
    }
    ...
    for (auto const& joint : model.joints_) {
      ...
      subscriptions_.push_back(
          this->create_subscription<sensor_msgs::msg::JointState>(
              "/" + joint.first + "/joint_states", rclcpp::SystemDefaultsQoS(),
              std::bind(&CANopenJointStatePublisher::topic_callback, this,
                        _1)));
    }
  }
 ...
 private:
  void topic_callback(const sensor_msgs::msg::JointState::SharedPtr msg) {
    // ROS2 CANopen publishes a <joint_name>/joint_states with a vector of only
    // one element
    if (msg->name.size() == 1) {
      // Search index that is associated with the joint name
      auto it =
          std::find(joint_names_.begin(), joint_names_.end(), msg->name[0]);
